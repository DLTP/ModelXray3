...
function updateStatus(loss) {
    ...
    if (brainStatus) {
        brainStatus.innerHTML = `<strong>Mode:</strong> ${model.totalSteps > 0 ? 'Trained' : 'Untrained'} | 
...
function runPrediction() {
    ...
    function createMatrixGrid(data, label) {
        ...
        grid.style.gridTemplateColumns = `repeat(${matrix[0].length}, 35px)`;
...
function createMatrixGrid(data) {
    const wrapper = document.createElement("div");
    wrapper.className = "matrix-scroll-wrapper";
    ...
    // Detect rows/cols
    let rows = Array.isArray(data[0]) ? data.length : 1;
    let cols = Array.isArray(data[0]) ? data[0].length : data.length;
...
if (matrix[0].length > MAX_COLS) {
    matrix = matrix.map(row => row.slice(0, MAX_COLS));
}
...
function saveDatasetToCSV() {
    ...
    let csv = "a,op,b,result\n";
    activeDataset.forEach(row => {
        csv += `${row.raw.n1},${row.raw.op},${row.raw.n2},${row.raw.res}\n`;
    });
...
function loadDatasetFromCSV(event) {
    ...
    reader.onload = function(e) {
        ...
        lines.forEach(line => {
            if (!line.trim()) return;
            const [a, op, b, result] = line.split(",");

            activeDataset.push({
                input: [parseFloat(a)/inputNorm, parseFloat(b)/inputNorm, ...encodeOp(op)],
                target: parseFloat(result)/outputNorm,
                raw: { n1: +a, n2: +b, op: op, res: +result }
            });
        });

        document.getElementById('dataset-status').innerText =
            `Loaded ${activeDataset.length} rows`;
    };
